...
- Repo linking: `ak link --project <name>` binds local repo to project. Stored in `~/.agent-kanban/links.json`. Auto-adds repository.
- Data model: Project is the workspace unit. Board is 1:1 with project. Repositories belong to projects. Tasks belong to boards, optionally linked to a repository. Machines belong to owner (user/org).
