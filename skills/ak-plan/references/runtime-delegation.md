...
If multiple runtimes are schedulable for the needed role and the user has not expressed a runtime preference, ask which runtime to use before creating new workers or assigning tasks. Present only runtimes with `runtime_available: true`, plus the relevant trade-off: existing matching workers, current load, model preference, or runtime-specific capability.

Do not ask when there is only one schedulable runtime, the user already specified a runtime, or existing matching workers are clearly the best choice by role and load.
...
Create workers by generating an Agent YAML from the current project context.
